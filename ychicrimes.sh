...
function usage() { 
	...
	echo	"$(tput setab 0)	-b Prints the adresse of crimes using Maps API                                    $(tput sgr 0)"
...
function filter(){
	...
	awk -v type="$t" -v desc="$d" -v year="$y" -v arr="$ar" -v domes="$dom" 'BEGIN {
		FPAT = "([^,]*)|(\"[^\"]+\")"}
			{print}' "$infile"
	else
...
#### Checking  if the filtered file is empty (contain just the header) before any action ##########################################################
